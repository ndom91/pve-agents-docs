...
| Agent credentials | the controller database, one per configured agent | `~/.config/agent-env`, sourced by the runner |
...
**What is used: reusable login state**, held in the controller database — one row per configured
agent, in the [Agents tab](/docs/reference/agents) — and written into each workspace as
`~/.config/agent-env`, which the runner sources explicitly. It arrives over stdin, never as an
argument. For Claude Code that is a subscription OAuth token; an API key was available and was
deliberately not chosen.

The shell variable each agent reads is the harness's own — `CLAUDE_CODE_OAUTH_TOKEN`,
`OPENCODE_CREDENTIAL` — rather than one controller-wide name. opencode's is a JSON envelope rather
than a token, because opencode accepts a credential through no API at all: its runner writes a row
into opencode's own database, which opencode reads on the next start.
