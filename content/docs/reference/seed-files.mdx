...
**Which destinations merge is the agent's own answer**, not a rule in the seeding code. Claude Code
names this one file; opencode names none, because it writes no configuration during bootstrap and so
has nothing to protect. An agent with no such file says no to everything, which is the honest
default rather than an omission. See [agents and harnesses](/docs/concepts/agents).
