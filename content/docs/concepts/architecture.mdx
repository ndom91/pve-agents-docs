...
checks the repository out into it, starts a coding agent inside it, and gives that agent the purpose
verbatim. The container is an implementation detail of that.

Which agent is part of the request rather than a property of the controller. There is no default:
...
**Agent control.** A runner process inside each workspace holds one coding-agent session
...
Which coding agent that is, is the workspace's own choice rather than the controller's. What differs
between one agent and the next lives behind a single interface — see
[agents and harnesses](/docs/concepts/agents) — and everything above that line is the same for all
of them.
