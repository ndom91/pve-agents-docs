...
go, rust
ripgrep, fd, jq
codex, claude-code, opencode, opencode2
...
One template runs every agent. `opencode2` is the v2 beta and installs beside v1 rather than over
it, so both are on PATH under their own names. A template missing an agent's binary produces
workspaces that provision cleanly and fail at the runner, which is why this is checked rather than
assumed.
